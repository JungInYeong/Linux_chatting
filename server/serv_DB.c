...
void login_ID(int clnt_socket, char packet[]) {
    ...
    if (mysql_real_connect(con, "127.0.0.1", "in", "in", "Test2_db", 3333, NULL, 0) == NULL) {
        finish_with_error(con);
    }
    ...
    char query[INFOSIZE];
    snprintf(query, sizeof(query), "SELECT id FROM users WHERE id='%s'", clnt_id);
...
void login_PW(int clnt_socket, char packet[]) {
    ...
    // 패킷에서 ID와 PW 추출
    sscanf(packet + 3, "%s %s", clnt_id, clnt_pw);

    // MYSQL에서 id와 pw 조회
    char query[INFOSIZE];
    snprintf(query, sizeof(query), "SELECT id FROM users WHERE id='%s' AND password='%s'", clnt_id, clnt_pw);
...
void save_Info(int clnt_socket, char packet[]) {
    ...
    // 패킷에서 ID와 PW 추출
    sscanf(packet + 4, "%s %s", clnt_id, clnt_pw);

    char query[INFOSIZE];
    snprintf(query, sizeof(query), "INSERT INTO users (id, password) VALUES('%s', '%s')", clnt_id, clnt_pw);
